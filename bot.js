...
bot.on('ready', function(data)
{
	...
	bot.roomInfo(false, function(info)
	{
		if (info.success)
		{
			...
			{
				AddActiveDJ(djs[i]);
				if (Config.DebugMode) {console.log('[INFO] Adding ' + djs[i] + ' to active DJs array.'); }
			}
		}
	});
});
...
bot.on('rem_dj', function(data)
{
	if (data.success)
	{
		...
		if (Config.DebugMode) { console.log('[INFO] User ' + data.user[0].name + ' stopped DJing.'); }
	}
});
...
bot.on('endsong', function(data)
{
	...
	if (data.room.metadata.current_song.metadata.album)
	{
		bot.speak(':loudspeaker: Last Played: \"' + data.room.metadata.current_song.metadata.song +
				...
				'\), which received ' + data.room.metadata.upvotes + ' upvotes and ' +
				data.room.metadata.downvotes + ' downvotes.');
	} 
	else 
	{
		bot.speak(':loudspeaker: Last Played: \"' + data.room.metadata.current_song.metadata.song +
				'\" by ' + data.room.metadata.current_song.metadata.artist + ', which received ' + data.room.metadata.upvotes + ' upvotes and ' +
				data.room.metadata.downvotes + ' downvotes.');
	}
	if (adjs.hasOwnProperty(data.room.metadata.current_dj))
	{
			adjs[data.room.metadata.current_dj].plays++;
			if (Config.DebugMode) { console.log('[INFO] ' + data.room.metadata.current_dj + ' has played ' + adjs[data.room.metadata.current_dj].plays + ' times.'); }
...
function RemoveActiveDJ(uid)
{
	if (adjs && uid && adjs.hasOwnProperty(uid))
	{
		adjs[uid].removed = new Date();
	}
}
